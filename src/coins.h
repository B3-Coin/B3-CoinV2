...
// (pre-BIP34) cases.
//! Add the outputs of `tx` as coins. With `exclude_modern_cells` (set by the
//! caller exactly when the block is in the B3 modern era), well-formed Modern
//! metadata cells and asset BURN-policy outputs are NOT added: they are
//! consensus-committed but never spendable. DisconnectBlock applies the
//! symmetric skip.
void AddCoins(CCoinsViewCache& cache, const CTransaction& tx, int nHeight,
              bool check = false, uint32_t nTxOffset = 0, bool exclude_modern_cells = false);
