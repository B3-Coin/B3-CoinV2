...
BOOST_AUTO_TEST_CASE(outputs_grouping_tests)
{
    ...
    const CTxDestination dest = *Assert(wallet->GetNewDestination(TEST_OUTPUT_TYPE, ""));
    for (unsigned long i = 0; i < GROUP_SIZE; i++) {
        addCoin(group_verifier.coins_pool, *wallet, dest, 10 * COIN, /*is_from_me=*/true);
    }

    group_verifier.GroupAndVerify(TEST_OUTPUT_TYPE,
                                  BASIC_FILTER,
    ...
    const CTxDestination dest2 = *Assert(wallet->GetNewDestination(TEST_OUTPUT_TYPE, ""));
    for (unsigned long i = 0; i < GROUP_SIZE; i++) {
    ...
    const CTxDestination dest3 = *Assert(wallet->GetNewDestination(TEST_OUTPUT_TYPE, ""));
    addCoin(group_verifier.coins_pool, *wallet, dest3, 1, true, CFeeRate(100));
    BOOST_CHECK(group_verifier.coins_pool.coins[TEST_OUTPUT_TYPE].back().GetEffectiveValue() <= 0);
    ...
    const CTxDestination dest4 = *Assert(wallet->GetNewDestination(TEST_OUTPUT_TYPE, ""));
    ...
    const CTxDestination dest5 = *Assert(wallet->GetNewDestination(TEST_OUTPUT_TYPE, ""));
    ...
    const CTxDestination dest7 = *Assert(wallet->GetNewDestination(TEST_OUTPUT_TYPE, ""));
