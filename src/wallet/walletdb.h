...
extern const std::string B3_FLOWMESH_BLS_KEY;
extern const std::string B3_FLOWMESH_BLS_CRYPTED_KEY;
...
public:
    ...
    //! FlowMesh's independent BIP340 trading account (secret in pk() descriptor).
    bool WriteB3FlowMeshAccountPubKey(const CPubKey& pubkey);
    ...
    //! FlowMesh seat keys are a collection keyed by their on-chain BLS
    //! public key. The two forms are mutually exclusive per public key.
    bool WriteB3FlowMeshBlsKey(const std::array<unsigned char, 48>& pubkey,
                               const std::vector<unsigned char>& secret);
    bool WriteB3FlowMeshBlsCryptedKey(
