...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " b3coin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The b3coin-tx tool is used for creating and modifying B3Coin transactions.\n\n"
                "b3coin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded B3Coin transaction, or with \"-create [commands]\" to create a hex-encoded B3Coin transaction.\n"
                "\n"
                "Usage: b3coin-tx [options] <hex-tx> [commands]\n"
                "or:    b3coin-tx [options] -create [commands]\n"
                "\n";
