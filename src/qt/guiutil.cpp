...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    // No example recipient: never suggest a Bitcoin address or a real payee.
    widget->setPlaceholderText(QObject::tr("Enter a B3 address"));
...
{
    // return if URI is not valid or is no b3coin: URI (the scheme the
    // rest of the client — formatBitcoinURI, payment server IPC — uses)
    if(!uri.isValid() || uri.scheme() != QString("b3coin"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("b3coin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("b3coin-%s.desktop", ChainTypeToString(chain)));
}
