...
BOOST_AUTO_TEST_CASE(BinaryOperatorTest)
{
    ...
    BOOST_CHECK(b >= a);
    BOOST_CHECK(b >= b);
    // a should be 0.000000002 B3/kvB now
    a += a;
    BOOST_CHECK(a == b);
}
...
BOOST_AUTO_TEST_CASE(ToStringTest)
{
    CFeeRate feeRate;
    feeRate = CFeeRate(1);
    BOOST_CHECK_EQUAL(feeRate.ToString(), "0.000000001 B3/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeRateFormat::BTC_KVB), "0.000000001 B3/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeRateFormat::SAT_VB), "0.001 base unit/vB");
}
