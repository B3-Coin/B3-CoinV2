...
struct MinerTestingSetup : public TestingSetup {
    MinerTestingSetup()
        : TestingSetup{ChainType::REGTEST,
                       {.extra_args = {
                            "-testactivationheight=bip34@227931",
                            "-testactivationheight=dersig@363725",
                            "-testactivationheight=cltv@388381",
                            "-testactivationheight=csv@419328",
                            "-testactivationheight=segwit@481824",
                        }}}
    {
        // Keep the original main-like subsidy boundary exercised below while
        // using non-B3 regtest for the generic block-assembly tests.
...
static constexpr CAmount TEST_BLOCK_SUBSIDY{50 * COIN};
...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vout.resize(1);
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - 1000;
    ...
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - 10000;
    ...
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - 1000 - 50000; // 50k base-unit fee
    ...
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - 1000 - 50000; // 0 fee
    ...
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - 1000 - 50000 - feeToUse;
    ...
    tx.vout.resize(2);
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - COIN;
    tx.vout[1].nValue = COIN; // One-coin output
    ...
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - COIN - feeToUse;
    ...
    tx.vout[0].nValue = COIN - 10000; // 10k base-unit fee
...
void MinerTestingSetup::TestPrioritisedMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vin[0].scriptSig = CScript() << OP_1;
    tx.vout.resize(1);
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY; // 0 fee
    ...
    tx.vin[0].prevout.hash = hashParentTx;
    tx.vout[0].nValue = TEST_BLOCK_SUBSIDY - 1000 - 1000; // 1000 base-unit fee
