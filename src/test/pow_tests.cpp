...
/**
 * The following retarget vectors are Bitcoin Core regression vectors. Keep
 * them independent from B3Coin mainnet, whose preserved legacy era uses its
 * own per-block hybrid PoW/PoS target calculation.
 */
Consensus::Params BitcoinMainnetPowTestParams()
{
...
{
    const auto consensus = BitcoinMainnetPowTestParams();
...
void sanity_check_chainparams(const ArgsManager& args, ChainType chain_type)
{
    ...
    BOOST_CHECK_EQUAL(consensus.hashGenesisBlock, chainParams->GenesisBlock().GetHash(consensus, /*height=*/0));
