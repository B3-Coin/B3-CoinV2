...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "b3coin-gui" : "b3coin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "b3coin-node" : "b3coind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("b3coin-cli");
