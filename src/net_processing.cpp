...
/** Historical B3Coin peers answer one getblocks request with at most 500 inventory entries. */
static constexpr size_t LEGACY_GETBLOCKS_RESPONSE_SIZE{500};
/**
 * Bound all queued legacy inventory, including unsolicited announcements.
 * This is deliberately larger than one getblocks page so a continuation
 * announcement cannot silently evict the tail of a page the peer will not
 * reannounce on this connection.
 */
static constexpr size_t MAX_LEGACY_BLOCK_QUEUE_SIZE{MAX_INV_SZ};
/**
 * Legacy getdata requests kept in flight per peer. Old peers serve getdata
 * inventories sequentially, so a window this size keeps blocks streaming
 * back-to-back instead of costing one round-trip per block, while staying
...
/** Cheap wire admission is separate from actual BLS work: known duplicates
 * cannot exhaust the verification budget, but still have a bounded CPU cost. */
static constexpr double MAX_FINALITY_WIRE_RATE_PER_SECOND{128.0};
static constexpr double MAX_GLOBAL_FINALITY_WIRE_RATE_PER_SECOND{512.0};
static constexpr size_t MAX_FINALITY_WIRE_TOKEN_BUCKET{2 * MAX_FINALITY_SIG_TOKEN_BUCKET};
static constexpr size_t MAX_GLOBAL_FINALITY_WIRE_TOKEN_BUCKET{2 * MAX_GLOBAL_FINALITY_SIG_TOKEN_BUCKET};
...
// Internal stuff
namespace {
std::chrono::microseconds FinalityTransportNow()
{
    return std::chrono::duration_cast<std::chrono::microseconds>(
        MockableSteadyClock::now().time_since_epoch());
}

void RefillFinalityBucket(double& bucket, std::chrono::microseconds& updated,
...
struct Peer {
    ...
    std::chrono::microseconds m_finality_sig_token_timestamp GUARDED_BY(NetEventsInterface::g_msgproc_mutex){FinalityTransportNow()};
    double m_finality_wire_token_bucket GUARDED_BY(NetEventsInterface::g_msgproc_mutex){MAX_FINALITY_WIRE_TOKEN_BUCKET};
    std::chrono::microseconds m_finality_wire_token_timestamp GUARDED_BY(NetEventsInterface::g_msgproc_mutex){FinalityTransportNow()};
     ...
     * downloads complete blocks through getblocks/inv/getdata, keeping a
     * window of getdata requests in flight so the download is pipelined
    ...
    */
    ...
    std::set<uint256> m_legacy_block_queue_set GUARDED_BY(NetEventsInterface::g_msgproc_mutex);
    /**
     * Number of blocks at the front of m_legacy_block_queue with a getdata
     * request outstanding. Entries stay queued until their requested block is
     * received, so a timed-out request can be retried without losing the
     * batch.
     */
    ...
    /**
     * Effective mode negotiated at VERSION: true when either endpoint
     * advertised the historical B3Coin protocol family (80000..80008). A
     * property of this connection only — distinct from the chain having
     * legacy history, from any candidate block's consensus era, and from the
     * codec of any particular block message.
     */
    std::atomic<bool> m_legacy_protocol{false};

    /**
     * Whether this node advertised the historical 80008 banner on this
     * connection. This is separate from m_legacy_protocol because either
     * endpoint can select the effective legacy mode. It is also separate from
     * m_reconnect_at_legacy_boundary: a post-H archival node advertises 80008
     * when replying to an inbound historical peer but keeps that connection
     * open to serve sealed history.
     */
    std::atomic<bool> m_our_legacy_version{false};

    /**
     * Whether our VERSION was selected as legacy because our own active tip
     * was still below the boundary. Such a connection must be recycled when
     * we cross H. This is deliberately distinct from m_our_legacy_version:
     * a post-H archival node replies 80008 to an inbound historical peer for
     * handshake compatibility, but must keep that serving connection open.
     */
    std::atomic<bool> m_reconnect_at_legacy_boundary{false};
...
public:
    ...
    void ActiveTipChange(const CBlockIndex& new_tip, bool) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_tx_download_mutex, !m_peer_mutex);
...
private:
    ...
    */
    ...
    void PushNodeVersion(CNode& pnode, Peer& peer,
                         bool reply_to_legacy_peer = false);
    ...
    std::chrono::microseconds m_finality_sig_global_token_timestamp GUARDED_BY(g_msgproc_mutex){FinalityTransportNow()};
    double m_finality_wire_global_token_bucket GUARDED_BY(g_msgproc_mutex){MAX_GLOBAL_FINALITY_WIRE_TOKEN_BUCKET};
    std::chrono::microseconds m_finality_wire_global_token_timestamp GUARDED_BY(g_msgproc_mutex){FinalityTransportNow()};
    ...
    bool ConsumeFinalityVerificationBudget(Peer& peer, std::chrono::microseconds now)
        EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);
    bool ConsumeFinalityWireBudget(Peer& peer, std::chrono::microseconds now)
        EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);
    ...
    /**
     * Legacy block inventory is connection-scoped: old peers will not
     * reannounce a getblocks page on the same connection. Keep one owner for
     * the serial full-block downloader so competing peers cannot create
     * duplicate, out-of-order windows. A disconnect releases the owner and
...
void PeerManagerImpl::PushNodeVersion(CNode& pnode, Peer& peer,
                                      const bool reply_to_legacy_peer)
{
    ...
    const int protocol_version{
        local_legacy_phase || reply_to_legacy_peer
            ? legacy::P2P_PROTOCOL_VERSION
            : b3_network ? B3_MODERN_PROTOCOL_VERSION : PROTOCOL_VERSION};
    peer.m_our_legacy_version.store(
        protocol_version == legacy::P2P_PROTOCOL_VERSION,
        std::memory_order_relaxed);
    peer.m_reconnect_at_legacy_boundary.store(local_legacy_phase,
                                               std::memory_order_relaxed);
...
void PeerManagerImpl::ActiveTipChange(const CBlockIndex& new_tip, bool is_ibd)
{
    ...
    // Keep the lock-free legacy-phase mirror current for the connection
    // thread. VERSION cannot be renegotiated in place. Every connection whose
    // 80008 banner was selected by our pre-H phase must therefore reconnect
    // exactly when this node crosses H; upgraded peers then negotiate the
    // modern banner, while historical peers remain legacy-only. A post-H
    // archival node serving history to a still-pre-H peer is intentionally
    // kept: its 80008 compatibility reply was not selected by our own phase.
    const bool legacy_phase{IsLegacyPhaseAt(&new_tip)};
    ...
    if (crossed_boundary) {
        ...
        std::set<NodeId> renegotiate;
        {
            ...
            for (const auto& [id, peer] : m_peer_map) {
                if (peer->m_reconnect_at_legacy_boundary.load(
                        std::memory_order_relaxed)) {
                    renegotiate.insert(id);
                }
            }
        }
        m_connman.ForEachNode([&](CNode* node) {
            if (!renegotiate.contains(node->GetId())) return;
            LogDebug(BCLog::NET,
                     "Disconnecting pre-boundary peer=%d to renegotiate the modern B3 protocol\n",
                     node->GetId());
...
void PeerManagerImpl::RelayFinalitySignatures(std::span<const node::FinalitySig> sigs)
{
    ...
    m_connman.ForEachNode([&](CNode* pnode) {
        if (!pnode->fSuccessfullyConnected || pnode->fDisconnect || pnode->fPauseSend) return;
        for (const node::FinalitySig& sig : sigs) {
            if (pnode->fPauseSend) break;
...
bool PeerManagerImpl::ConsumeFinalityVerificationBudget(Peer& peer,
                                                       const std::chrono::microseconds now)
{
    RefillFinalityBucket(peer.m_finality_sig_token_bucket, peer.m_finality_sig_token_timestamp, now,
                        MAX_FINALITY_SIG_RATE_PER_SECOND, MAX_FINALITY_SIG_TOKEN_BUCKET);
    RefillFinalityBucket(m_finality_sig_global_token_bucket, m_finality_sig_global_token_timestamp, now,
...
bool PeerManagerImpl::ConsumeFinalityWireBudget(Peer& peer,
                                               const std::chrono::microseconds now)
{
    RefillFinalityBucket(peer.m_finality_wire_token_bucket, peer.m_finality_wire_token_timestamp, now,
                        MAX_FINALITY_WIRE_RATE_PER_SECOND, MAX_FINALITY_WIRE_TOKEN_BUCKET);
    RefillFinalityBucket(m_finality_wire_global_token_bucket, m_finality_wire_global_token_timestamp, now,
                        MAX_GLOBAL_FINALITY_WIRE_RATE_PER_SECOND, MAX_GLOBAL_FINALITY_WIRE_TOKEN_BUCKET);
    if (peer.m_finality_wire_token_bucket < 1.0 || m_finality_wire_global_token_bucket < 1.0) return false;
    peer.m_finality_wire_token_bucket -= 1.0;
    m_finality_wire_global_token_bucket -= 1.0;
    return true;
}
                                            ...
                                            const std::chrono::microseconds now)
{
    ...
    const size_t peer_count{WITH_LOCK(m_peer_mutex, return m_peer_map.size())};
    {
        ...
        if (retry_due && m_pending_finality_sigs.Size() > 0) {
            ...
            const auto resolver = [&](const node::PendingFinalitySignatures::Entry& entry) -> std::optional<uint256> {
                ...
                return entry.sig.index < size ? digest : std::nullopt;
            };
            const auto available = [&](const int64_t source) { return source_peer(source) != nullptr; };
            ...
            for (const auto& entry : m_pending_finality_sigs.TakeReadyWithSources(ready_height, now, resolver, available, budget)) {
                // TakeReady charged exactly one surviving source. Re-run all
                // ordinary pool validation, without charging BLS a second time.
                // This bounded retry reserves a token conservatively even if
                // Submit can now deduplicate a vote verified since admission.
    ...
    for (const auto& entry : accepted) {
        m_connman.ForEachNode([&](CNode* other) {
            if (other->GetId() == entry.peer || !other->fSuccessfullyConnected || other->fDisconnect || other->fPauseSend) return;
...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    // The raw witness-block fast path below bypasses the decoded-block guard.
    // Apply the same protocol boundary from the index header first: an
    // effective legacy connection can serve history through H, but cannot
    // decode a marker-modern block regardless of the requested inventory
    // flavor.
    if (peer.m_legacy_protocol && Consensus::HasB3BlockCodecV2(pindex->nVersion)) {
    ...
    if (pblock) {
        ...
        assert(!peer.m_legacy_protocol || !Consensus::HasB3BlockCodecV2(pblock->nVersion));
...
bool PeerManagerImpl::MaybeSendGetHeaders(CNode& pfrom, const CBlockLocator& locator, Peer& peer)
{
    if (WITH_LOCK(cs_main, return IsLegacyPhase())) {
        ...
        // a pipelined window of getdata requests in flight.
        const auto current_time = NodeClock::now();
        if (peer.m_legacy_blocks_in_flight > 0 &&
            current_time - peer.m_legacy_request_time > HEADERS_RESPONSE_TIME) {
            // Legacy peers remember every inventory item they have announced
            // to us, so restarting with getblocks would permanently lose
            // these blocks. Retry the exact getdata requests instead.
            LogDebug(BCLog::NET, "Retrying %zu stalled legacy B3Coin getdata requests to peer=%d\n",
                     peer.m_legacy_blocks_in_flight, pfrom.GetId());
            std::vector<CInv> invs;
            invs.reserve(peer.m_legacy_blocks_in_flight);
            for (size_t i = 0; i < peer.m_legacy_blocks_in_flight; ++i) {
                invs.emplace_back(MSG_BLOCK, peer.m_legacy_block_queue[i]);
            }
            MakeAndPushMessage(pfrom, NetMsgType::GETDATA, invs);
            peer.m_legacy_request_time = current_time;
            return true;
        }
        // Top the in-flight window up from the queued inventory.
        const size_t target{std::min(peer.m_legacy_block_queue.size(), LEGACY_BLOCK_DOWNLOAD_WINDOW)};
        if (target > peer.m_legacy_blocks_in_flight) {
            std::vector<CInv> invs;
            invs.reserve(target - peer.m_legacy_blocks_in_flight);
            for (size_t i = peer.m_legacy_blocks_in_flight; i < target; ++i) {
                invs.emplace_back(MSG_BLOCK, peer.m_legacy_block_queue[i]);
            }
            peer.m_legacy_blocks_in_flight = target;
            peer.m_legacy_request_time = current_time;
            MakeAndPushMessage(pfrom, NetMsgType::GETDATA, invs);
            return true;
        }
        if (peer.m_legacy_blocks_in_flight > 0) {
            // Window is full and within its response time; blocks are
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        const bool acceptable_services{remote_legacy_protocol
        ...
        if (pfrom.IsInboundConn()) {
            PushNodeVersion(pfrom, peer, remote_legacy_protocol);
        }

        const bool legacy_protocol_peer{
            remote_legacy_protocol ||
            peer.m_our_legacy_version.load(std::memory_order_relaxed)};
        ...
        // Potentially mark this peer as a preferred download peer.
        {
            ...
            // A post-H node may keep an effective-legacy connection solely to
            // serve a lagging historical peer. It cannot provide modern
            // headers or blocks, so counting it as preferred would suppress
            // usable inbound peers and could stall modern synchronization.
            const bool protocol_can_sync_local_tip{
                m_in_legacy_phase.load(std::memory_order_relaxed) ||
                !legacy_protocol_peer};
            state->fPreferredDownload =
                protocol_can_sync_local_tip &&
        ...
        // The final Bitcoin alert is not meaningful to the legacy B3Coin
        // network, which has its own alert identity.
        if (!m_chainparams.GetConsensus().legacy_b3coin && greatest_common_version <= 70012) {
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                if (legacy_chain) {
                    // Keep the old getblocks/getdata exchange attached to one
                    ...
                    if (!fAlreadyHave) {
                        ...
                        // recovered on this connection.
                        if (peer.m_legacy_block_queue_set.contains(inv.hash)) continue;
                        if (peer.m_legacy_block_queue.size() >= MAX_LEGACY_BLOCK_QUEUE_SIZE) {
                            LogWarning("Legacy B3Coin inventory queue full for peer=%d; disconnecting to retry from a fresh peer\n",
                                       pfrom.GetId());
                            pfrom.fDisconnect = true;
                            return;
                        }
                        if (peer.m_legacy_block_queue_set.insert(inv.hash).second) {
                            peer.m_legacy_block_queue.push_back(inv.hash);
                        }
                    }
                    continue;
                }
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        int nLimit = 500;
        const std::optional<int> legacy_last_height{
            peer.m_legacy_protocol.load(std::memory_order_relaxed)
                ? Consensus::LegacyFinalHeight(m_chainparams.GetConsensus())
                : std::nullopt};
        LogDebug(BCLog::NET, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), nLimit, pfrom.GetId());
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
            // A historical peer can decode and request blocks only through
            // the pinned legacy boundary. Advertising marker-modern hashes
            // would make its final getblocks page end in requests we must
            // refuse, leaving the old client waiting after it already reached
            // all serveable history.
            if (legacy_last_height && pindex->nHeight > *legacy_last_height) {
                LogDebug(BCLog::NET,
                         "  getblocks stopping at legacy boundary %d for peer=%d\n",
                         *legacy_last_height, pfrom.GetId());
                break;
            }
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        // A legacy peer normally serializes getdata responses. Receiving a
        // requested child before the queue head cannot be validated without a
        // block cache, and treating it as progress would indefinitely defer
        // retrying the missing parent. Disconnect and let a fresh sync peer
        // restart from the connected tip instead.
        if (legacy_block_requested && !legacy_block_is_next) {
            LogWarning("Legacy B3Coin peer=%d delivered requested block %s out of order; disconnecting\n",
                       pfrom.GetId(), hash.ToString());
            pfrom.fDisconnect = true;
            return;
        }
        ...
        if (b3_missing_parent) {
            ...
            }
            {
                ...
                if (legacy_block_requested) {
                    ...
                    // every orphan arrival into an immediate full-window
                    // getdata retry against the peer.
                    if (!peer.m_legacy_block_queue.empty() && peer.m_legacy_block_queue.front() == hash) {
                        peer.m_legacy_block_queue_set.erase(hash);
                        peer.m_legacy_block_queue.pop_front();
                        if (peer.m_legacy_blocks_in_flight > 0) {
                            --peer.m_legacy_blocks_in_flight;
                        }
                    }
                    peer.m_legacy_request_time = NodeClock::now();
                    if (peer.m_legacy_blocks_in_flight > 0) {
                        // Let the rest of the window stream in; the last
                        // arrival restarts discovery below.
                        return;
                    }
                    // The remaining queue descends from a base that cannot
                    // connect; drop it and restart discovery from the
                    // connected tip.
        ...
        const bool legacy_block{prev_block && Consensus::GetB3Era(prev_block->nHeight + 1, m_chainparams.GetConsensus()) == Consensus::B3Era::LEGACY};
        ...
        if (legacy_block_requested) {
            ...
            // delivery (another peer supplied this block first) must still
            // pop this peer's request, or its window wedges permanently and
            // re-downloads the same blocks on every stall retry.
            if (!peer.m_legacy_block_queue.empty() && peer.m_legacy_block_queue.front() == hash) {
                peer.m_legacy_block_queue_set.erase(hash);
                peer.m_legacy_block_queue.pop_front();
                if (peer.m_legacy_blocks_in_flight > 0) {
    ...
    if (msg_type == NetMsgType::FINSIG) {
        ...
        // single BLS verification, inside the pool. Cheap wire-admission
        // buckets bound work before cs_main; separate budgets are charged only
        // immediately before BLS verification, not for known duplicates. There is no peer
        ...
        const auto now{FinalityTransportNow()};
        if (!ConsumeFinalityWireBudget(peer, now)) return;
        ...
        bool relay{false};
        {
            ...
            const auto accept{chainstate.FinalitySignatures().Submit(
                ...
                bridge_index, [&]() EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex) {
                    return ConsumeFinalityVerificationBudget(peer, now);
                })};
            ...
            if (accept == node::FinalitySignaturePool::Accept::TOO_SHALLOW ||
                accept == node::FinalitySignaturePool::Accept::VERIFICATION_DEFERRED ||
                (accept == node::FinalitySignaturePool::Accept::NOT_CHECKPOINT &&
                 finsig.height > static_cast<uint64_t>(tip->nHeight))) {
                // The bounded transport queue also covers a nearby block not
                // connected yet, and valid coordinates awaiting BLS budget.
                // Neither admission nor elapsed time grants signature validity.
                if (const auto candidate{node::InspectFinalityTransport(
                        finsig, tracker.Current(), chainstate.m_chain, consensus, bridge_index)}) {
                    if (m_pending_finality_sigs.Add(finsig, pfrom.GetId(), candidate->digest, now)) {
        ...
        if (relay) {
            m_finality_relay_dirty.store(true, std::memory_order_relaxed);
            m_connman.ForEachNode([&](CNode* pnode) {
                if (pnode->GetId() == pfrom.GetId() || !pnode->fSuccessfullyConnected || pnode->fDisconnect || pnode->fPauseSend) return;
    ...
    if (msg_type == NetMsgType::NOTFOUND) {
        ...
        if (m_chainparams.GetConsensus().legacy_b3coin) {
            LOCK(cs_main);
            if (m_legacy_sync_peer == pfrom.GetId()) {
                const size_t requested{std::min(peer.m_legacy_blocks_in_flight,
                                                 peer.m_legacy_block_queue.size())};
                for (const CInv& inv : vInv) {
                    if (!inv.IsMsgBlk()) continue;
                    for (size_t i{0}; i < requested; ++i) {
                        if (peer.m_legacy_block_queue[i] == inv.hash) {
                            LogWarning("Legacy B3Coin sync peer=%d cannot serve block %s; disconnecting for failover\n",
...
bool PeerManagerImpl::SendMessages(CNode& node)
{
    ...
    // Belt-and-suspenders for a handshake that was still in flight when H
    // connected: ActiveTipChange can only walk fully connected nodes, so the
    // next message pass closes any connection whose VERSION was selected by
    // our pre-H phase. A post-H 80008 reply to an inbound historical peer is
    // a stable compatibility connection and is deliberately excluded.
    if (!m_in_legacy_phase.load(std::memory_order_relaxed) &&
        peer.m_reconnect_at_legacy_boundary.load(
            std::memory_order_relaxed)) {
        node.fDisconnect = true;
        return true;
    }
    ...
    MaybeFinalityTransport(node, peer, FinalityTransportNow());

    {
        ...
        if (IsLegacyPhase() && CanServeBlocks(peer) &&
        ...
        } else if (!peer.m_legacy_protocol && !state.fSyncStarted &&
                   CanServeBlocks(peer) && !m_chainman.m_blockman.LoadingBlocks()) {
