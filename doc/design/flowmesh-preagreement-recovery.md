...
## 2026-09-19 delivery-stall repair (historical first-run status)

At the time of this first run the repair was uncommitted and pending independent
audit. The follow-up below and `flowmesh-v1-integrated-qualification.md` record
the subsequent integration and qualification; earlier failures remain evidence.
...
of 176 distinct messages in the failed slot). Duplicate amplification exhausted
the existing admission budget; the repair preserves the configured limits.
