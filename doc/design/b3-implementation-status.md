...
| Bridge-backed bUSD | **B3 CONSENSUS PATH IMPLEMENTED; MAINNET FAIL-CLOSED PENDING INDEPENDENT GATES** | Canonical Ethereum-mainnet USDT (6 decimals) is bound to managed-v1 vault `0x143F207e23e6aebD7E974be90ac6D434f4c7BFb6`. A strict bounded type-10 codec implements bootstrap, update, mint, execution backfill, and managed withdrawal. Every record requires exactly one zero-value policy-9 `BRIDGE_RECORD` metadata output committing its canonical frame; ordinary `SIGHASH_ALL` signatures cover that output, with no `OP_RETURN` or custom sighash. Consensus verifies the finalized Ethereum receipt and exact log, requires the exact OWNER mint, enforces nullifiers and block/epoch caps, records an exact bUSD BURN request under stable `(Txid, burn-vout)` identity, and wires authorization through asset conservation, mempool, miner, block validation, undo, and reindex. Candidate validation uses base-plus-overlay state; recent undo is bounded and deeper reorgs trigger deterministic replay. State is in memory and rebuilt from activation; configured bridge nodes refuse pruning and snapshots that skip bridge history because no durable sidecar exists. Mainnet remains off pending adapter enforcement, reproducible runtime/audits, operator Ethereum-release automation and durable request consumption, and production checkpoint/fork/cap/approval/activation/rules/X pins. Later decentralization requires a new audited vault and vault-bound asset migration. |
