...
command), ~~wire `TrustedReplay` into the node~~ (done — pinned-boundary blocks connect
through the engine; admission, read-back and fork choice are replay-scoped, with
off-anchor tip recovery); remaining: run `b3coin-utxo-verify` against real `master`
state at the candidate H (operator step, needs a real synced datadir).
