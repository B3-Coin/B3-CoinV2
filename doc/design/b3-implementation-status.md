...
| H is a hard finality boundary | **BLOCKER** | Predicate `Consensus::DisconnectCrossesLegacyBoundary` (`src/consensus/era.h`) is enforced in `Chainstate::DisconnectTip`, but `ActivateBestChainStep` converts a refused disconnect into `FatalError` → node shutdown, and `FindMostWorkChain` never filters cross-boundary candidates. A peer presenting a heavier fork rooted ≤ H aborts the node and it re-aborts on restart. Contract §3 requires **rejection of the candidate**, not local failure. Tracked as **D1**. |
...
| Item | Status | Evidence / gap |
|---|---|---|
| Cross-boundary reorg handling | **BLOCKER** | **D1**, see §1 above. |
| Work/chainwork gating scoped correctly | **BLOCKER** | **D2**: `HasValidProofOfWork` returns true unconditionally under `legacy_b3coin`; the low-work header gate and `TryLowWorkHeadersSync` are disabled chain-wide and never re-enable after a boundary is set. Because legacy `nBits` is validated only at connect time, a peer can synthesize PoS-shaped blocks with arbitrary claimed chainwork and no work, and get them into the block index and onto disk. **Constraint (contract §58): the fix must not impose PoW validity on historical B3 PoS blocks** — legacy PoS legitimately carries no PoW; scope the gates instead. |
...
| Cross-era reorg test matrix | PARTIAL | Most rows of contract §60 are covered by `legacy_boundary_tests`, `legacy_identity_tests` and `legacy_transition_tests`. The real `DisconnectTip` reorg path (D1) is **untested**. |
