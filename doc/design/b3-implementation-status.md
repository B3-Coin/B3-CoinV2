...
| Cross-era reorg test matrix | PARTIAL | Most rows of contract §60 are covered by `legacy_boundary_tests`, `legacy_identity_tests` and `legacy_transition_tests`, and the D1 fork-point predicate is covered directly. **Still owed:** an integration test that drives a real cross-boundary reorg *attempt* through `ActivateBestChain` and asserts the node rejects the candidate and keeps its tip. Constructing a heavier prohibited candidate currently depends on D2 (unvalidated `nBits` lets a single low-target block claim arbitrary chainwork), so this test should land with the D2 work to avoid encoding the defect it relies on. |
