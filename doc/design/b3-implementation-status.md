...
| Bounded, progress-safe sync | PARTIAL | **D3 core done.** The legacy sync owner now holds a 120s forward-progress lease (`LEGACY_SYNC_PROGRESS_LEASE`) independent of the per-request retry timer, renewed only when a requested block connects to the active chain. On expiry the owner is released (ordinary stall, not scored) and barred from immediately reclaiming, so a stalled/silent owner no longer wedges sync. Orphan-arrival discovery restart is now owner-only. Sync-target state is trustworthy (row above). Covered by `legacy_net_tests`. **Still owed:** a hard cap on the empty-inv completion latch (`m_legacy_sync_complete`) so an owner cannot end sync prematurely, and the `NODE_WITNESS` outbound-selection issue that makes legacy peers unreachable after their real services are recorded. |
