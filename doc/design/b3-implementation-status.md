...
| Work/chainwork gating scoped correctly | PARTIAL | **D2 fixed (header path).** `AcceptBlockHeader` now makes the legacy era **blocks-only**: a legacy-codec header presented without its block is refused (`legacy-header-only`) and creates no index entry, so untrusted legacy headers gain no chain-selection weight before validation (`src/validation.cpp`, `full_block` flag). This does not impose PoW on legacy PoS or constrain `nBits`; the trust formula is unchanged (see [b3-legacy-fork-choice.md](b3-legacy-fork-choice.md)). Covered by the transition fixture. **Still owed (follow-on hardening):** port the historical rolling depth bound (`CheckSync`/`nCheckpointSpan = 500`) and the hardened checkpoint list, both legacy-era consensus rules, to bound deep-fork *full-block* spam. A full block with a fake-hard `nBits` still enters the index with high `nChainWork` before `ConnectBlock` rejects it; the anchor-ineligibility check keeps it off the active chain, and `ConnectBlock` fails it, but the depth bound is the historical mechanism that stops it being stored at all. |
| Bounded, progress-safe sync | **BLOCKER** | **D3**: legacy sync ownership can be claimed by an unauthenticated inbound peer, which can then wedge sync permanently (empty-inv completion latch, or never replying) while other peers' block announcements are discarded. No stall timeout, no eviction, no misbehavior scoring on legacy paths. A specific ownership policy is *not* locked; the requirement is bounded and progress-safe (contract §59). |
...
| Cross-era reorg test matrix | PARTIAL | Most rows of contract §60 are covered by `legacy_boundary_tests`, `legacy_identity_tests` and `legacy_transition_tests`. The D1 fork-point predicate is covered directly; `IsAnchorIneligible` classification (off-anchor side branch ineligible and never a candidate; canonical prefix, X, and modern descendants eligible) is covered in the transition fixture, together with the D2 header-only rejection. **Still owed:** an end-to-end test that drives a *heavier* prohibited candidate through `ActivateBestChain` to exercise the `BLOCK_ANCHOR_INELIGIBLE` marking path and confirm no `FatalError`. A legacy side branch cannot out-work the chain to X structurally, so such a candidate requires a crafted block with fake-hard `nBits` (and a forged PoS shape to bypass the scrypt check); once the depth bound above lands, that scenario changes again. Deferred with the depth-bound hardening. |
