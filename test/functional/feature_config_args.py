...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'b3coin.conf'
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'Data directory "{self.nodes[0].datadir_path}" contains a "b3coin.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in b3coin.conf or in an included conf file
        ...
        with open(self.nodes[0].datadir_path / 'b3coin.conf', 'a') as conf:
    ...
    def test_config_file_log(self):
        ...
        # Write a b3coin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.bitcoinconf.read_text()
        conf_path = default_datadir / "b3coin.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a b3coin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "b3coin.conf" file which is ignored, because a '
        ...
        # the same b3coin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/b3coin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when b3coin.conf in the default data directory sets another datadir '
                      'and it contains a different b3coin.conf file that would be ignored')
        ...
        # b3coin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "b3coin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "b3coin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/b3coin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "b3coin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "b3coin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_b3coin.conf"
