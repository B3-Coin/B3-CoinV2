...
def get_binary_paths(config):
    ...
    binaries = {
        ...
        "b3coin-cli": "BITCOINCLI",
        "b3coin-util": "BITCOINUTIL",
        "b3coin-tx": "BITCOINTX",
        "b3coin-chainstate": "BITCOINCHAINSTATE",
        "b3coin-wallet": "BITCOINWALLET",
    }
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "b3coin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "b3coin.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "b3coin.conf")):
        with open(os.path.join(datadir, "b3coin.conf"), 'r') as f:
            for line in f:
